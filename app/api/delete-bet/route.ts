...
export async function GET(request: NextRequest) {
  ...
  const days = GRACE_DAYS_DEADLINE_HOST_WINNING_OUT;

  // USE |${days} * INTERVAL '1 DAY'| and not |INTERVAL '${days} DAYS'| because it doesn't work
  try {
    await prisma.$executeRaw`
      ...
      SET "outcomeTypeCode" = (
        SELECT id FROM "OutcomeType" WHERE code = '2'
          ...
          "outcomeDeadline" = NOW() + ${days} * INTERVAL '1 DAY'
